...
const ScheduleSection = async ({ eventList }: ScheduleSectionProps) => {
    const _schedule = await getSchedule();

    return (
        <div className={'m-5'}>
            {eventList.map((entry) => {
                const detailsParts = entry.details?.split('\n');
                const details = detailsParts?.map((part, i) => (
                ...
                return (
                    <div key={entry.date.toString()} className={'mb-10'}>
                        <h2 className={'text-2xl font-bold uppercase'}>
                            {entry.name}
                        </h2>
                        <div
                            className={
                                'flex md:space-x-8 flex-wrap md:flex-nowrap mt-5'
                            }
                        >
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <Link
                                                    ...
                                                    target={'_blank'}
                                                    className={
                                                        'hover:underline hover:text-emerald-600'
                                                    }
                                                >
                            ...
                            <div className={'mt-5 md:mt-0'}>{details}</div>
                        </div>
                    </div>
                );
            })}
