...
const Section = (props: SectionProps) => {
    ...
    return (
        <div
            className={`mx-auto max-w-(--breakpoint-lg) ${xPadding} ${yPadding} ${props.className}`}
        >
            <div className="flex items-center justify-between w-full p-5 mb-5 font-medium rtl:text-right rounded-t-xl gap-3">
                <span
                    className={`${collapsed ? 'text-left' : 'text-center w-full'}`}
                >
                    {props.title && (
                        <h2 className={`text-4xl font-bold ${titleStyle}`}>
                            {props.title}
                        </h2>
                    )}
                    {props.description && (
                        <div className={`mt-4 text-xl ${descriptionStyle}`}>
                            {props.description}
                        </div>
                    )}
                </span>
                {props.collapsible && (
                    ...
                    >
                        {collapsed ? (
                            <ChevronDownIcon
                                className={`${collapseButtonStyle} h-6 w-6 duration-75`}
                            />
                        ) : (
                            <ChevronDownIcon
